...
class PlayState extends MusicBeatState
{
	...
	// cascade shit
	var cascadeBack:FlxSprite;
	var cascadeGround:FlxSprite;

	// wechnia bg why there so many sprite AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	var wechniaBG:FlxSprite;
	var wechniaBGTreeThings:FlxSprite;
	var wechniaHoles:FlxSprite;
	var wechniaPurpleBalls:FlxSprite;
	var wechniaYellowShrooms:FlxSprite;
	var wechniaTrees:FlxSprite;
	var wechniaFloor:FlxSprite;
	var wechniaFloorShaded:FlxSprite;
	var wechniaRectangularPrisms:FlxSprite;
	var wechniaFG:FlxSprite;
	var wechniaGlow:FlxSprite;

	// 32x bg :3
	var thirtytwoxBG:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'entrance':
				...
				entranceBG.loadGraphic(Paths.image('entrance/bg'));
				...
				entranceClock.loadGraphic(Paths.image('entrance/clock'));
				...
				entranceIdk.loadGraphic(Paths.image('entrance/idk'));
				...
				entranceFloor.loadGraphic(Paths.image('entrance/floor'));
				...
				entranceOver.loadGraphic(Paths.image('entrance/over'));
				...
				entranceSpookyBG.loadGraphic(Paths.image('entrance/scary/bg2'));
				...
				entranceSpookyClock.loadGraphic(Paths.image('entrance/scary/clock2'));
				...
				entranceSpookyIdk.loadGraphic(Paths.image('entrance/scary/idk2'));
				...
				entranceSpookyFloor.loadGraphic(Paths.image('entrance/scary/floor2'));
				...
				entranceSpookyOver.loadGraphic(Paths.image('entrance/scary/over2'));
			...
			case 'emerald':
				...
				cascadeBack = new FlxSprite(-600, -550);
				cascadeBack.loadGraphic(Paths.image('emerald/ehzback'));
				cascadeBack.scrollFactor.set(0.7, 0.7);
				cascadeBack.setGraphicSize(Std.int(cascadeBack.width * 1.6));
				cascadeBack.updateHitbox();

				cascadeGround = new FlxSprite(-780, -500);
				cascadeGround.loadGraphic(Paths.image('emerald/EHZGROUND'));
				cascadeGround.frames = Paths.getSparrowAtlas('emerald/EHZGROUND');
				cascadeGround.animation.addByPrefix('exist', "EHZGROUND", 24, true);
				cascadeGround.animation.play('exist');
				cascadeGround.setGraphicSize(Std.int(cascadeGround.width * 2));
				cascadeGround.updateHitbox();

				add(cascadeBack);
				add(cascadeGround);

			case 'wechnia': // I wanna kms tryna make these stages man :<
				GameOverSubstate.characterName = 'bfii-death';

				wechniaBG = new FlxSprite(-600, -400);
				wechniaBG.loadGraphic(Paths.image('wechnia/1'));
				wechniaBG.scrollFactor.set(0.1, 0.1);
				
				wechniaBGTreeThings = new FlxSprite(-650, -400);
				wechniaBGTreeThings.loadGraphic(Paths.image('wechnia/2'));
				wechniaBGTreeThings.scrollFactor.set(0.3, 0.3);
				
				wechniaHoles = new FlxSprite(-700, -400);
				wechniaHoles.loadGraphic(Paths.image('wechnia/3'));
				wechniaHoles.scrollFactor.set(0.4, 0.4);

				wechniaPurpleBalls = new FlxSprite(-700, -450);
				wechniaPurpleBalls.loadGraphic(Paths.image('wechnia/4'));
				wechniaPurpleBalls.scrollFactor.set(0.7, 0.7);

				wechniaYellowShrooms = new FlxSprite(-700, -450);
				wechniaYellowShrooms.loadGraphic(Paths.image('wechnia/5'));
				wechniaYellowShrooms.scrollFactor.set(0.7, 0.7);

				wechniaTrees = new FlxSprite(-800, -300);
				wechniaTrees.loadGraphic(Paths.image('wechnia/6'));
				wechniaTrees.scrollFactor.set(0.8, 0.8);

				wechniaFloor = new FlxSprite(-750, -400);
				wechniaFloor.loadGraphic(Paths.image('wechnia/7'));
				wechniaFloor.scrollFactor.set(1, 1);

				wechniaFloorShaded = new FlxSprite(-750, -401);
				wechniaFloorShaded.loadGraphic(Paths.image('wechnia/8'));
				wechniaFloorShaded.scrollFactor.set(1, 1);
				
				wechniaRectangularPrisms = new FlxSprite(-800, -400);
				wechniaRectangularPrisms.loadGraphic(Paths.image('wechnia/9'));
				wechniaRectangularPrisms.scrollFactor.set(1.15, 1.15);

				wechniaFG = new FlxSprite(-840, -350);
				wechniaFG.loadGraphic(Paths.image('wechnia/10'));
				wechniaFG.scrollFactor.set(1.3, 1.3);

				wechniaGlow = new FlxSprite(-800, -300);
				wechniaGlow.loadGraphic(Paths.image('wechnia/11'));
				wechniaGlow.scrollFactor.set(0.4, 0.4);
				wechniaGlow.blend = ADD;

				add(wechniaBG);
				add(wechniaBGTreeThings);
				add(wechniaHoles);
				add(wechniaPurpleBalls);
				add(wechniaYellowShrooms);
				add(wechniaTrees);
				add(wechniaFloorShaded);
				add(wechniaFloor);

			case '32x':
				GameOverSubstate.characterName = 'mighty32x-dead';

				thirtytwoxBG = new FlxSprite(-600, -540);
				thirtytwoxBG.loadGraphic(Paths.image('32xbg'));
				thirtytwoxBG.frames = Paths.getSparrowAtlas('32xbg');
				thirtytwoxBG.animation.addByPrefix('exist', "frame", 8, true);
				thirtytwoxBG.animation.play('exist');
				thirtytwoxBG.setGraphicSize(Std.int(thirtytwoxBG.width * 4));
				thirtytwoxBG.updateHitbox();
				thirtytwoxBG.antialiasing = false;
		...
		theStatic.frames = Paths.getSparrowAtlas('staticc');
		...
		timeTxt = new FlxText(19, 19, 400, "", 32);
		timeTxt.setFormat(Paths.font("chaotix.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBar = new FlxBar(0, 0, LEFT_TO_RIGHT, 1280, 19, this, 'songPercent', 0, 1);
		if(ClientPrefs.downScroll)
			timeBar.y = 701;
		...
		songNameHUD = new FlxText(-19, 19, 1280, "", 32);
		songNameHUD.setFormat(Paths.font("chaotix.ttf"), 32, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		switch (daSong)
		{
			case  'breakout' | 'soulless-endeavors' | 'long-sky' | 'cascade' | 'color-crash' | 'next-level':
				add(blackFuck);
				startCircle.loadGraphic(Paths.image('openings/' + daSong + '_title_card'));
				startCircle.frames = Paths.getSparrowAtlas('openings/' + daSong + '_title_card');
	...
	function startSong():Void
	{
		...
		FlxTween.tween(songNameHUD, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'cascade':
			{
				switch (curStep)
				{
					...
					case 1920:
						defaultCamZoom = 1;
						FlxTween.tween(camHUD, {alpha: 0}, 0.4, {ease: FlxEase.cubeInOut});
					case 1936:
						defaultCamZoom = 0.7;
						holyFuckStopZoomin = true;
						FlxTween.tween(camHUD, {alpha: 1}, 1, {ease: FlxEase.cubeInOut});
